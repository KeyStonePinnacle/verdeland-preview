...
if(grid){
 function render(town){const list=homes.filter(h=>town==='all'||h.town===town);grid.innerHTML=list.map(h=>`<a class="home-card" href="${h.url}"><div class="photo"><img src="${h.images[0]||'assets/brand/model-placeholder.svg'}" alt="${h.images.length?'Exterior of '+h.title:'Photography pending for '+h.title}" width="1800" height="1200" loading="lazy"><span>${h.label}</span></div><div class="card-bottom"><div><h3>${h.title}</h3><p>${h.town}, New York</p></div><span class="card-arrow" aria-hidden="true">↗</span></div></a>`).join('');document.querySelector('#home-count').textContent=`${list.length} ${list.length===1?'home':'homes'}`}
...
const inquiry=document.querySelector('#inquiry');if(inquiry){document.querySelector('[data-inquiry]').addEventListener('click',()=>inquiry.showModal());inquiry.querySelectorAll('.close,[data-close]').forEach(b=>b.addEventListener('click',()=>inquiry.close()));
